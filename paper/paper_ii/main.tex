...
A region $K\subset\Omega$ is called metastable if the process remains in,
or repeatedly returns to, a localized memory-supported regime for many memory
times, and if the associated memory-induced drift remains comparable to the
stochastic fluctuations. This is an operational diagnostic rather than a
dimension-selection theorem.

\paragraph{Dimension-selection heuristic.}
Low-dimensional random motion is strongly recurrent, while very high ambient
dimension can dilute localized memory unless kernel scales, amplitudes, and
normalizations compensate for the available volume. These competing effects
motivate the search for an intermediate effective dimension, and they explain
why near-three-dimensional diagnostics are scientifically interesting.

The present repository, however, does not prove the classical heuristic as a
lemma. Recurrence is modified by self-interaction, the memory gradient need not
diverge for all admissible kernels, and large-dimensional dilution depends on
kernel normalization, deposition scale, memory mass, and coupling strength.
Consequently, the current status is a research programme:
\begin{equation}
\text{local memory shape} \quad\longrightarrow\quad
\text{external coarse dimension}
\end{equation}
remains to be established by controlled simulations and operator diagnostics.

\paragraph{Current numerical input.}
The corrected scalar long-run reference slice in the companion analysis shows
a compact co-moving memory cloud whose covariance participation dimension is
near three in the chosen three-dimensional embedding. For the strongest current
candidate the seedwise range is approximately
\begin{equation}
D_{\mathrm{mem}}\in[2.914,2.947],
\end{equation}
with memory-roundness in the range $0.809$--$0.849$, while the matched
$\eta=0$ controls are broader, less round, and lower-dimensional on average.
This supports a local shape diagnostic, not a derivation of three external
dimensions.

\paragraph{Remark (Scale dependence).}
Additional weakly coupled or internal modes may exist on microscopic
timescales,
\begin{equation}
\tau_{\mathrm{mode}}\ll\tau_{\mathrm{mem}}\sim\alpha^{-1},
\end{equation}
and need not contribute to macroscopic metastability. Paper~II therefore treats
effective dimension as scale- and observer-dependent until multi-knot response,
ambient-dimension robustness, and coarse-grained closure are demonstrated.

\subsection{Interim conclusion}
Physical space should not yet be identified with the abstract state space
$\Omega$ itself, nor has a unique $d=3$ macroscopic dimension been derived.
The defensible claim is narrower: stable, approximately isotropic,
low-dimensional memory-supported regimes can define candidate effective
coordinates. Whether the observed local three-dimensional memory shape becomes
a shared external spatial dimension is the central open task of this paper.
