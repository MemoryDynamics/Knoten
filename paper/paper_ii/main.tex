...
\title{Emergent Spacetime Kinematics from Irreversible Memory Dynamics}
...
\begin{abstract}
We propose a minimal irreversible, finite-memory dynamics in which effective space and relativistic kinematics arise as coarse-grained consistency conditions of metastable, localized structures (``dynamical knots''). We outline how effective dimensionality, a finite propagation speed, and Lorentz-like kinematics emerge without postulating spacetime at the microscopic level.
\end{abstract}
...
The present work adopts a deliberately minimal and dynamical notion of structure. No geometric or algebraic framework beyond an abstract continuous state space with local coarse-graining is assumed. In particular, spatial dimension, isotropy, and relativistic kinematics are not postulated but diagnosed operationally from the stability properties of metastable dynamical regimes. This places our approach conceptually close to a broad class of pre-geometric and emergent-geometry programs, while remaining distinct in its methodological commitments.
...
The central claim of this paper is that space and relativistic kinematics arise as consistency conditions of interacting, memory-driven dynamical structures. In particular:
\begin{itemize}
...
\item Finite memory and delayed self-interaction necessarily imply a finite propagation speed for information between distinct structures.
\item Requiring consistency of descriptions across different localized dynamical regimes leads to a Lorentz-like kinematical invariance at large scales \cite{Volovik2003Universe,Liberati2013Lorentz}.
\end{itemize}
...
We first establish an effective spatial interpretation of $\Omega$ (including the macroscopic selection $d=3$), then derive a finite and universal propagation speed $c_{\mathrm{eff}}$ from finite memory coupling, and finally obtain Lorentz-like kinematics from observer equivalence and invariant $c_{\mathrm{eff}}$.
...
The state at update step $n$ is given by $\sigma_n = (x_n,\rho_n)$, where
...
\textit{[TODO: Insert figure: knot stability vs. effective dimensionality / relaxation spectrum.]}
...
The selection of three macroscopic dimensions arises at an earlier, nonlinear stage of the memory-driven dynamics.
...
Consistency of the coupled dynamics therefore excludes instantaneous influence and enforces a finite response time between spatially separated knots.

\subsection{Effective maximal propagation speed}
The delayed memory response implies the existence of a maximal propagation speed for dynamical influence.
...
For multiple knots sharing a common memory substrate, consistency of their effective descriptions requires that the maximal propagation speed be universal.
Different values of $c_{\mathrm{eff}}$ would prevent a coherent comparison of spatial and temporal scales between knots.

Thus, a universal maximal propagation speed emerges dynamically as a consistency condition of interacting memory-driven structures.

\subsection{Interim conclusion}
Finite, relaxing memory generically enforces a finite and universal maximal propagation speed for information between metastable dynamical structures.
This result is independent of any spacetime or relativistic postulates and follows solely from the irreversible memory dynamics.
...
Section~\ref{sec:finite_speed} established the existence of a finite maximal propagation speed $c_{\mathrm{eff}}$, arising as a consistency condition of finite, relaxing memory.
...
\begin{enumerate}
\item the existence of a finite, universal maximal propagation speed $c_{\mathrm{eff}}$,
...
\subsection{Interim conclusion}
Lorentz kinematics emerges as the unique linear invariance structure compatible with a finite, universal propagation speed and observer equivalence in the metastable regime of memory-driven dynamics.
No microscopic spacetime assumptions are required.

\section{Outlook}\label{sec:outlook}

In this paper we have argued that effective space and relativistic kinematics need not be postulated at the microscopic level: they can arise as coarse-grained consistency conditions of an irreversible, finite-memory dynamics. Metastable localized structures (dynamical knots) furnish operational coordinates, finite memory enforces a universal maximal propagation speed $c_{\mathrm{eff}}$, and observer equivalence in the metastable regime selects Lorentz-like kinematics.
...
\begin{itemize}
\item \textbf{Minimal ingredients.} A finite memory persistence time and finite effective memory range are sufficient to exclude instantaneous influence and imply a maximal propagation speed $c_{\mathrm{eff}}$.
...
\item \textbf{Testable numerical diagnostics.} In simulations one can measure (i) the retarded response kernel $K_{AB}$ between knots, (ii) the emergent speed estimate $c_{\mathrm{eff}}\sim \ell_{\mathrm{mem}}/\tau_{\mathrm{mem}}$, and (iii) the stability spectrum of knots (and its relation to the effective dimension diagnostic).
\item \textbf{Open dynamical questions.} A next step is to derive how interactions between multiple knots renormalize $c_{\mathrm{eff}}$ and constrain admissible effective field content in the metastable regime.
\end{itemize}
