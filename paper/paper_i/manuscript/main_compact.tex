...
\label{eq:potential}
\end{align}
where $0<\lambda_{\mathrm m}<1$, $\beta_\rho,\varepsilon,\eta\geq0$, and
...
A separate finite-time test restored additive innovations to two prepared
$d=2$ finite-memory rotating waves.  At fixed memory time the comparison used
$\chi=\varepsilon/(R\sqrt{\alpha})$, so that
$D/R^2=\chi^2/2$.  Both cells and all three registered common-noise paths
passed their orbital gates for $10^{-15}\leq\chi\leq10^{-4}$; $\chi=10^{-3}$
and $10^{-2}$ failed the preregistered local phase and chirality gates, while
the radius and transverse-deviation gates remained well inside threshold.
Values through $\chi=10^{-16}$ were unresolved in binary64.  This is a
finite-time phase-coherence bracket, not an interpolated critical point or a
visible orbit-disintegration threshold.  Raw $\varepsilon$ is not comparable
across cells or parameter conventions, and the test establishes neither noisy
formation nor a physical noise scale.
