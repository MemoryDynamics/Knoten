...
A minimal reference implementation accompanies this work in the repository, with a NumPy-based finite-memory simulator, diagnostic functions, and reproducible runner scripts (`scripts/smoke_test.py` and `scripts/reference_experiment.py`) that support the reproducibility of the key numerical claims.
...
A small NumPy reference implementation in `src/emergenz_knoten` and reproducible runner scripts (`scripts/smoke_test.py` and `scripts/reference_experiment.py`) provide a concrete basis for verifying the numerical diagnostics reported in this draft.
