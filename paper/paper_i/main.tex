...
In regimes where such candidates occur, we define trajectory-based, coordinate-robust diagnostics (persistence/relaxation rates, dimensionless stability measures, and metastable residence-time statistics) to identify and compare localized memory-cloud structures induced by the endogenous memory coupling.
Calibration to spacetime units, physical energy or mass scales, relativistic dynamics, or Standard-Model-like structures is outside the scope of the present paper.
\end{abstract}
...
Several programs in fundamental physics investigate whether structures such as spacetime, locality, fields, or quantum states could be effective rather than primitive.
Examples include asymptotic safety \cite{Weinberg1979,Reuter1998,NiedermaierReuter2006,Percacci2017}, causal sets \cite{Bombelli1987,Sorkin2003}, loop-based formulations \cite{Rovelli2004,AshtekarLewandowski2004}, and string theory \cite{Polchinski1998}; broader emergence arguments also motivate asking which structures must be postulated and which can be reconstructed operationally \cite{Anderson1972,Laughlin2000}.

The present paper does not attempt such a reconstruction.
It isolates a smaller mathematical question: what follows from a point-valued stochastic update coupled to a self-generated memory that relaxes?
Concretely, we assume a sequence of non-trivial update events defining a trajectory in an abstract state space, together with an explicit memory variable that compresses the ordered past.
...
Memory and trajectory are generated jointly as the minimal departure from trivial dynamics, rather than being encoded in external boundary conditions.
...
Subsequent sections analyze memory-induced irreversibility and then develop operational diagnostics (metastability, persistence/relaxation rates, and dimensionless stability measures) for localized metastable candidates.
...
%-------------------------------------------------
\subsection{Minimalist motivation: from directed updates to effective coordinates}\label{sec:minimalist_motivation}
%-------------------------------------------------
...
The coarse-grained coordinate and metric language serves only as a consistency target and an interpretive guide for diagnostics extracted from trajectories.
...
$\beta$. The normalized convention used below sets
$\lambda_{\mathrm m}=\beta=\alpha$, in
...
\end{equation}
where $0<\lambda_{\mathrm m}<1$, $\beta\ge 0$, and $G_\sigma$ is a smooth,
normalized kernel with finite width $\sigma$. The normalized convention used
for the rest of this paper is $\lambda_{\mathrm m}=\beta=\alpha$, in which the
...
In the regimes where a continuum description is meaningful, the next question is whether the dynamics self-organizes into metastable regions $\mathcal{K}\subset\Omega$.
...
Having established an effective notion of time, we now turn to memory-induced localized structures and their relaxation-based stability diagnostics.
...
\subsection{Numerical status and claim boundary}\label{sec:numerical_status}

The numerical role of this paper is diagnostic rather than theorem-proving.
In the current corrected-sign scalar reference slice, five-seed runs of length $N=3\times10^7$ show a co-moving compact memory-cloud regime separated from the matched $\eta=0$ control.
For the strongest current slice ($A_{\mathrm{att}}=35$ in the two-scale kernel convention used by the implementation), the median co-moving radius is approximately $2.1\times10^{-4}$, compared with about $10^{-3}$ in the $\eta=0$ control, and the radius-normalized center drift is about $0.044$, compared with about $0.320$ in the control.
The weighted memory cloud in the chosen three-dimensional embedding has covariance participation dimension $D_{\mathrm{mem}}\simeq2.94$ and roundness about $0.84$, compared with $D_{\mathrm{mem}}\simeq1.90$ and roundness about $0.45$ in the corresponding control.
These numbers support the limited statement that scalar memory feedback can generate seed-stable, co-moving compact memory-cloud candidates in this parameter slice.

This should not be read as a dimension-selection result.
Here $D_{\mathrm{mem}}$ is a memory-cloud shape diagnostic in the chosen coordinate scaffold, not a proof that physical space has three dimensions.
Ambient-dimension audits in the repository indicate that strict memory-variance diagnostics such as $D_{\mathrm{mem}}$ and the number of axes required to capture 90--95\% of memory variance grow with the supplied embedding dimension.
At the same time, spectral/coarse memory diagnostics show near-three-dimensional behavior in some high-dimensional runs.
We therefore treat the local three-dimensional memory shape as a diagnostic in the chosen embedding and as motivation for later tests of external/coarse dimensionality, not as a conclusion of the present minimal model.
...
\subsection{Relaxation scale as confinement diagnostic}

The stability of a dynamical knot is governed by the rate at which perturbations decay.
We therefore characterize a knot by a relaxation or confinement scale, rather than by introducing an inertial parameter.
...
\end{equation}
with more strongly confined knots corresponding to larger $\Gamma_{\mathrm{rel}}$ (faster relaxation against fluctuations).
...
A sharper identification with a physical energy or mass scale requires (and is deferred to later work) a geometric calibration and an operational mapping to physical units.
...
\paragraph{Terminology (possible mass interpretation).}
No physical mass is introduced in this paper.
The quantity defined here is a coarse-grained, quasi-time-independent property of a metastable attractor regime, quantified by how rapidly perturbations relax back into the knot.
If later calibrated work uses mass-like language for this scale, that terminology should be understood as closer to ``stiffness'' or ``confinement strength'' than to Newtonian inertia; a mapping to conventional energy/mass units requires an additional calibration.
...
A calibration of these scales to familiar physical constants and units is deferred to later work.
...
Here, $R$ is only an operational drift-to-fluctuation diagnostic; a fuller phase-level interpretation is deferred to later work.
...
In this formulation, no mass parameter is introduced; the operational proxy is a measure of the stability of an attractor against stochastic perturbations.
More strongly confined structures correspond, operationally, to steeper memory-induced potentials and/or stronger memory coupling, and thus faster relaxation.
...
Interpreting such stability scales as confinement proxies is natural at the level of the effective description, while any identification with physical units, mass scales, or relativistic structure is deferred to later work.
...
Finally, extending the framework to multiple coupled generators and exploring whether consistent universal scales can be defined are natural next steps.
