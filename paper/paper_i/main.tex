...
\title{Minimal Point-Process Dynamics with Finite Memory: Markov Embedding and Metastable Structures}
...
\begin{abstract}
We propose a minimal stochastic dynamics on an abstract continuous state space $\Omega$ in which a visible trajectory $x_n$ is coupled to an explicit, finite, exponentially relaxing memory density $\rho_n$.
The marginal process $\{x_n\}$ is generally non-Markovian, while the augmented state $\sigma_n=(x_n,\rho_n)$ is Markov by construction.
...
Any calibration to spacetime units, physical mass scales, relativistic dynamics, or Standard-Model-like structures is deferred to companion work.
\end{abstract}
...
While our model shares with these approaches the idea of a trajectory coupled to a self-generated field, it differs structurally: the memory is an explicit, finite, exponentially relaxing distribution updated by Eq.~(\ref{eq:memorydistribution}).
Consequently, the visible coordinate $x_n$ alone is generally non-Markovian, since its next update depends on the retained memory field, whereas the augmented state $\sigma_n=(x_n,\rho_n)$ is Markov by construction.
This Markov embedding is the main mathematical anchor of Paper~I.
The same memory update is generically non-invertible (information-losing) and thereby induces an intrinsic arrow of \emph{update order} at the fundamental level; that is, the full past cannot be reconstructed from the present state.
...
The broader placement of this construction within self-interacting diffusions, self-repelling polymers, and Markovian embeddings of memory processes is a natural subject for a separate mathematical companion note; the present paper keeps only the model definition and operational diagnostics needed for the minimal construction.
...
A small reference implementation accompanies this work in the project repository and includes a finite-memory simulator, diagnostic functions, tests, and a reproducible reference runner.
...
First, once the extended state is defined in Eq.~(\ref{eq:sigma}), the resulting process is Markov by construction, and the update order $n\mapsto n+1$ is physically meaningful because the memory update is generally non-invertible.
...
Given $\sigma_n$ and a fresh noise draw $\xi_n$, the distribution of $\sigma_{n+1}$ is determined by Eqs.~(\ref{eq:trajectory}) and~(\ref{eq:memorydistribution}).
This is the Markovian description used in Paper~I.
...
We use $n\in\mathbb{N}$ for the discrete update index and reserve the word ``time'' for the coarse-grained parameter introduced later (Sec.~\ref{sec:EmergenceTime}). In Eq.~(\ref{eq:sigma}), $x_n\in\Omega$ is the current process position and $\rho_n$ is a normalized, non-negative \emph{memory density} on $\Omega$ (with respect to the reference measure $dx$). While $\rho_n$ is normalized like a probability density, in Paper~I it is primarily a bookkeeping device for finite, relaxing memory; its normalization fixes an overall scale and should not be read as an epistemic probability unless explicitly stated.
...
It is also the finite-memory embedding that makes the model Markov in the augmented state: the exponentially weighted past is represented by the current field $\rho_n$ rather than by retaining the full history as a primitive object.
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.72\linewidth]{fig1_knot_trajectory.pdf}
    \caption{
...
We work exclusively with the update index $n$ and the ordered state sequence $\sigma_n$.
...
Because the memory update is non-invertible, the state sequence in Eq.~(\ref{eq:sigma})
\[
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{fig_Gamma_alpha.pdf}
    \caption{
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{fig_alpha.pdf}
    \caption{
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{fig3_knot_trajectory.pdf}
    \caption{
...
A conceptual caveat: the extended state $\sigma_n=(x_n,\rho_n)$ is Markov by construction, but the marginal process $\{x_n\}$ is generally non-Markovian because it depends on memory.
...
\begin{itemize}
\item $\alpha$ is estimated from the exponential decay of memory weights in the linear update, i.e. by fitting the age-dependent factor $(1-\alpha)^m\simeq e^{-\alpha m}$ or by fitting the decay of a memory-sensitive observable such as $\Phi[\rho_n](x_n)$.
\item $D$ is the effective diffusion coefficient of the coarse-grained dynamics, with the standard scaling estimate $D\sim\varepsilon^2/(2\alpha)$ in the continuum limit.
...
\item $\widehat{\Gamma} := (\sigma^2/D)\,\Gamma_{\mathrm{rel}}$ is a dimensionless stability measure that compensates for uniform coordinate rescalings of the effective state space.
\end{itemize}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{fig2_knot_scatter.pdf}
    \caption{
...
We introduced a minimal self-interacting stochastic dynamics with finite, exponentially relaxing memory.
