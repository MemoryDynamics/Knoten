...
While our model shares with these approaches the idea of a trajectory coupled to a self-generated field, it differs structurally: the memory is an explicit, exponentially relaxing distribution updated by Eq.~(\ref{eq:memorydistribution}).
...
This Markov embedding is the main mathematical anchor of the present paper and is consistent with the standard observation that finite-memory stochastic processes can be represented as Markov processes on an enlarged state space \cite{NoeNuske2013,NoeWuPrinzPlattner2013}.
...
First, once the extended state is defined in Eq.~(\ref{eq:sigma}), the resulting process is Markov by construction, and the update order $n\mapsto n+1$ is dynamically meaningful because the relaxing memory state compresses the ordered past.
...
\subsection{Transition kernel and observable semigroup}

Let $\Sigma$ denote the augmented state space of pairs $\sigma=(x,\rho)$.
Because the fresh noise draw is sampled at each update, the one-step dynamics is most naturally represented by a Markov transition kernel rather than by a deterministic map:
\begin{equation}
P(\sigma,A)
:=
\mathbb{P}\{\sigma_{n+1}\in A\mid \sigma_n=\sigma\},
\qquad
A\subseteq\Sigma .
\end{equation}
...
(Uf)(\sigma)
:=
\int_\Sigma f(\sigma')\,P(\sigma,d\sigma')
=
\mathbb{E}[f(\sigma_{n+1})\mid\sigma_n=\sigma].
\end{equation}
...
The absence of a two-sided inverse should therefore be understood carefully.
...
\begin{figure}[!tbp]
    ...
    Once $\sigma_n=(x_n,\rho_n)$ is used as the state, all past influence enters through $\rho_n$, while fresh randomness enters through $\xi_n$.
    }
...
We therefore work with the update index $n$ and the ordered state sequence $\sigma_n$.
...
The state sequence in Eq.~(\ref{eq:sigma})
\[
...
An operator-theoretic refinement is available once augmented features along the trajectory are recorded.
For a chosen lag and a coarse partition or basis on $\Sigma$, candidate knots can be tested as metastable or almost-invariant sets of the transfer operator, equivalently as slowly decaying modes separated by a spectral gap \cite{FroylandLloydSantitissadeekorn2010,NoeNuske2013}.
The present paper uses residence weights and relaxation fits as trajectory-level diagnostics; a full transfer-operator/MSM analysis is left to subsequent work.
...
In a global transfer-operator description at lag $\tau$, the same idea would be expressed through a slow eigenvalue $\mu_i(\tau)$ via
\begin{equation}
