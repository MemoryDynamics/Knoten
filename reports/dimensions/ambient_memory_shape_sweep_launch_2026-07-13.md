...
PowerShell does not use `\` as a line-continuation character. Use the following PowerShell-native argument-array form from the repository root:

```powershell
foreach ($dim in 4,5,7,10,13,20) {
  $outDir = "data/processed/long_run_metastability/ambient_dim_memory_shape_Aatt_35_N30M_d${dim}_seed1-5_eps1em4_2026-07-13"
  New-Item -ItemType Directory -Force -Path $outDir | Out-Null
  (Get-Date -Format o) | Set-Content -Path "$outDir\run.started" -Encoding ASCII
  $argsList = @(
    'experiments/current/dynamics/long_run_metastability.py',
    '--steps', '30000000',
    '--dim', "$dim",
    '--seeds', '1,2,3,4,5',
    '--conditions', 'baseline,eta_zero',
  ...
  & .\.venv\Scripts\python.exe @argsList 1> "$outDir\run.stdout.log" 2> "$outDir\run.stderr.log"
  $LASTEXITCODE | Set-Content -Path "$outDir\run.exitcode" -Encoding ASCII
}
```

## Post-Run Aggregation

After completion, aggregate with this PowerShell-native command:

```powershell
$argsList = @(
  'experiments/current/dynamics/ambient_dimension_memory_shape_report.py',
