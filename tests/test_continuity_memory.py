...
def test_partly_neutral_mode_is_marginal_not_unstable() -> None:
    mode = continuity_memory_mode(
        0.0,
        memory_relaxation=0.0,
        flux_relaxation=0.5,
        stiffness=1.0,
    )
    assert mode.classification == "marginal_real"
    assert not mode.asymptotically_stable
    np.testing.assert_allclose(np.sort_complex(mode.eigenvalues), [-0.5, 0.0])
