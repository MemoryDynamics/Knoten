...
def ensemble_rms(alpha, eta, eps, sigma, base_seed, N_ens, T, N_mem, T_mid):
    ...
    for k in prange(N_ens):
        _, r2 = single_traj_midpoint(
            alpha, eta, eps, sigma, base_seed + k, T, N_mem, T_mid
        )
        s += r2
...
@njit(parallel=True, fastmath=True)
def ensemble_stats(
    alpha, eta, eps, sigma, base_seed, N_ens, T, N_mem, times, T_mid, r0
):
    n_times = len(times)
...
def analyze_pair(args):
    ...
    # 1) rms
    rms = ensemble_rms(alpha, eta, eps, sigma, base_seed, N_ens, T, N_mem, T_mid)
    r0 = 0.2 * rms

    # 2) P0(t) + Kovarianz
    counts, sum_x, sum_xxT = ensemble_stats(
        alpha, eta, eps, sigma, base_seed + 999999, N_ens, T, N_mem, times, T_mid, r0
    )
