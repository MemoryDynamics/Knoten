"""Small reference simulations for the finite-memory model."""
...
def simulate_finite_memory(config: SimulationConfig, *, seed: int = 0) -> dict[str, np.ndarray]:
    ...
    It is meant for smoke tests, diagnostics development, and sanity checks.
    """
    ...
    horizon = min(config.max_memory, max(1, int(config.memory_factor / config.alpha)))
...
def simulate_finite_memory_numba(config: SimulationConfig, *, seed: int = 0) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "final_x": np.zeros(config.dim, dtype=float),
        "memory": np.zeros((0, config.dim), dtype=float),
