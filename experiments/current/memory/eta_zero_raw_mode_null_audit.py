...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("reports/memory/eta_zero_raw_mode_null_audit_2026-07-31.md"),
    )
    parser.add_argument(
        "--summary-json",
        type=Path,
        default=Path("reports/memory/eta_zero_raw_mode_null_audit_2026-07-31.json"),
    )
    parser.add_argument(
        "--figure",
        type=Path,
...
def _report(payload: dict[str, Any], report: Path, figure: Path) -> str:
    ...
    lines += [
        ...
        "## Reproduction",
        "",
        "    python experiments/current/memory/eta_zero_raw_mode_null_audit.py",
        "",
