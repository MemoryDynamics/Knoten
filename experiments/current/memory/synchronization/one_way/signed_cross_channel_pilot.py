...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "reports/response/scalar/signed_scalar_cross_channel_pilot_2026-07-18.json"
        ),
    )
    parser.add_argument(
        "--figure",
...
def build_report(payload: dict[str, Any], report_path: Path, figure: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "- Script: experiments/current/memory/synchronization/one_way/signed_cross_channel_pilot.py",
            "",
        ]
    )
    return "\n".join(lines)
