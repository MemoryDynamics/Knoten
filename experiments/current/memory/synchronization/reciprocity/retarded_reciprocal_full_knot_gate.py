...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "reports/response/reciprocal/retarded_reciprocal_full_knot_gate_2026-08-04.md"
        ),
    )
    parser.add_argument(
        "--summary-json",
        type=Path,
        default=Path(
            "reports/response/reciprocal/retarded_reciprocal_full_knot_gate_2026-08-04.json"
        ),
    )
    parser.add_argument(
        "--figure",
...
def _report(payload: dict[str, Any], report: Path, figure: Path) -> str:
    ...
    lines = [
        ...
        f"- {len(payload['rows'])} condition-common, node-specific future-noise seeds, {parameters['updates']:,} updates = {derived['continuation_memory_times']:.1f} memory times, first {derived['analysis_start_memory_times']:.1f} excluded.",
        "",
    ...
    lines.extend(
        [
            ...
            "- command: `python experiments/current/memory/synchronization/reciprocity/retarded_reciprocal_full_knot_gate.py`;",
