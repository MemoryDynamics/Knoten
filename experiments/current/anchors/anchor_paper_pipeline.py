...
def run_case(args: argparse.Namespace, *, alpha: float, seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "transfer": {
            ...
            "stationary_eigenvalues_reported": int(
                np.sum(np.isclose(transfer.eigenvalues, 1.0 + 0.0j))
            ),
            "unit_modulus_modes_reported": int(
                np.sum(np.isclose(np.abs(transfer.eigenvalues), 1.0))
            ),
            "eigenvalues": _complex_summary(transfer.eigenvalues),
            "relaxation_eigenvalues": _complex_summary(
                transfer.relaxation_eigenvalues
            ),
