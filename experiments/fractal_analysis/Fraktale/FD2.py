...
def fractal_dimension(points):
    ...
    sizes = np.logspace(np.log10(max_range / 200), np.log10(max_range / 5), 12)
...
def single_run(args):
    ...
    traj = simulate(
        N=300_000, dim=dim, eps=eps, eta=2, alpha=alpha, A=1, B=B, s1=1, s2=0.15
    )
...
def dimension_test():
    ...
    for d in dims:
        ...
        for _ in range(3):
            traj = simulate(
                N=900_000, dim=d, eps=eps, eta=2, alpha=alpha, A=1, B=B, s1=1, s2=0.15
            )
