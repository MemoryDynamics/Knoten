...
def horizon(config: ArchiveFractalConfig) -> int:
    ...
    return min(config.max_memory, max(1, int(config.memory_factor / config.alpha) + 1))
...
def summarize_runs(runs: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for run in runs:
        key = (str(run["condition"]), int(run["steps"]))
        groups.setdefault(key, []).append(run)

    summary = []
    for (condition, steps), group in sorted(
...
- embedding dimension: `{metadata["base_config"]["dim"]}`
- seeds: `{metadata["seeds"]}`
- steps ladder: `{metadata["steps_list"]}`
- conditions: `{metadata["conditions"]}`
- engine request: `{metadata["engine_request"]}`
- sample_every: `{metadata["base_config"]["sample_every"]}`
- max_memory: `{metadata["base_config"]["max_memory"]}`
    ...
    for steps in args.steps_list:
        for condition in args.conditions:
            for seed in args.seeds:
                print(f"running condition={condition} steps={steps} seed={seed}")
                runs.append(
                    run_case(
