...
A minimal reference implementation accompanies this work in the repository, with a NumPy-based finite-memory simulator, diagnostic functions, tests under `tests/`, and the reproducible runner `experiments/reference_experiment.py` that support the reproducibility of the key numerical claims.
...
A small NumPy reference implementation in `src/emergenz_knoten`, tests under `tests/`, and the reproducible runner `experiments/reference_experiment.py` provide a concrete basis for verifying the numerical diagnostics reported in this draft.
