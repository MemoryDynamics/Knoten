...
flowchart LR
    ...
    p5d["P5-D three calls inconclusive<br/>result pipeline closed"]
...
`allow_nan=False`. A later, newly remediated attempt 3 completed the in-memory
panel but failed its first strict v2 validation on finite `numpy.float64`
zeros generated by the production Channel-off energy floor. All three calls
are inconclusive, the attempt-3 authorization is exhausted and interaction
evidence remains absent. Relaxation-diffusion,
...
flowchart LR
    ...
    interaction["P5 three calls inconclusive<br/>result pipeline closed"]
...
Rohartefakt und keine beobachtete Entscheidung. Nach einer separaten
Runner-Remediation und neuer Einmalfreigabe berechnete Versuch 3 das
vollstaendige In-memory-Panel, scheiterte jedoch vor jeder Publikation am
strikten v2-Typvertrag fuer sechs produktionsspezifische
`numpy.float64`-Nullquotienten. Nur das Attempt-3-Receipt existiert; Ergebnis,
Report, Manifest, Audit und beobachtete Entscheidung fehlen. Die Freigabe ist
